...
// =======================================================
// TOKEN MIPASAJE (MPJ)
// 1 MPJ = 1 PASAJE DE TRANSPORTE
// Blockchain: Stellar (Soroban)
// Uso: Control de pasajes y prevención de colados
// =======================================================
...
/// TokenInterface: Trait que define la interfaz del token MIPASAJE según CAP-46
pub trait MiPasajeTrait {

    /// Inicializa el token MIPASAJE con metadatos y admin
    fn initialize(
        env: Env, 
        admin: Address, 
        name: String, 
        symbol: String,
        decimals: u32
    ) -> Result<(), MiPasajeError>;

    /// ADMIN - FUNCTIONS(VENDER Y VALIDAR PASAJES)
    /// Crea nuevos tokens cuando se venden pasajes
    fn mint(env: Env, to: Address, amount: i128) -> Result<(), MiPasajeError>;
    
    /// Destruye tokens cuando un pasaje es usado (validacion al subir)
    fn burn(env: Env, from: Address, amount: i128) -> Result<(), MiPasajeError>;
    
    /// USER - FUNCTIONS (TRANSFERIR PASAJES)
    /// Consulta el balance de MPJ de una cuenta
    fn balance(env: Env, account: Address) -> i128;
    
    /// Transferir pasajes (tokens) entre usuarios
    fn transfer(
        env: Env, 
        from: Address, 
        to: Address, 
    ...
    /// Aprueba a un spender para gastar tokens en nombre del owner, osea es aprobar que otro usuario gaste mis pasajes
    fn approve(
        env: Env, 
        from: Address, 
        spender: Address, 
    ...
    /// Transfiere MPJs usando allowance previamente aprobado
    fn transfer_from(
        env: Env, 
        spender: Address, 
        from: Address, 
...
/// Estructura del contrato MiPasajeTrait BDB
#[contract]
pub struct TokenMiPasaje;

/// Implementación del contrato, crea el client que implementa MiPasajeTrait
#[contractimpl]
impl MiPasajeTrait for TokenMiPasaje {
    fn initialize(
        env: Env, 
        admin: Address, 
        name: String, 
        ...
        // 1. Verificar que TokenMiPasaje no esté ya inicializado
        if env.storage().instance().has(&DataKey::Initialized) {
            return Err(MiPasajeError::AlreadyInitialized);
        }
        
        // 2. Validar decimales (máximo 18 como Ethereum)
        if decimals > MAX_DECIMALS || decimals != 0 {
            return Err(MiPasajeError::InvalidDecimals);
        }
        
        // 3. Validar metadatos (name y symbol no vacíos)
        // Nota: String en Soroban no tiene .len() directo, pero podemos convertir a bytes para validar
        if name.len() == 0 || name.len() > MAX_NAME_LENGTH {
            return Err(MiPasajeError::InvalidMetadata);
        }
    ...
    // Emisión de nuevos pasajes (tokens) 
    fn mint(env: Env, to: Address, amount: i128) -> Result<(), MiPasajeError> {
        // 1. Verificar inicialización
        if !env.storage().instance().has(&DataKey::Initialized) {
            return Err(MiPasajeError::NotInitialized);
        }
        ...
        // 3. Validaciones
        if amount <= 0 {
            return Err(MiPasajeError::InvalidAmount);
        }
        ...
        let new_balance = balance.checked_add(amount)
            .ok_or(MiPasajeError::OverflowError)?;
    ...
    fn burn(env: Env, from: Address, amount: i128) -> Result<(), MiPasajeError> {
        ...
        if balance < amount {
            return Err(MiPasajeError::InsufficientBalance);
        }
    ...
    // Cuantos pasajes (tokens) tiene un pasajero (cuenta)
    fn balance(env: Env, account: Address) -> i128 {
    ...
    fn transfer(
        ...
        if from == to {
            return Err(MiPasajeError::InvalidRecipient);
        }
    ...
    fn transfer_from(
        ...
        if allowed < amount {
            return Err(MiPasajeError::InsufficientAllowance);
        }
